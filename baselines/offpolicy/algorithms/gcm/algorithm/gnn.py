...
from baselines.offpolicy.algorithms.utils.gnn import EmbedConv
...
class GNNBase(nn.Module):
    def __init__(self, args: argparse.Namespace, 
        ...
        # Note: SimplifiedAttentionNet sums heads if not concat? 
        # Wait, SimplifiedAttentionNet implementation above sums heads:
        # x = (head_outputs * alpha.unsqueeze(1)).sum(dim=-1)
        # So out_dim is hidden_size.
        # But GNNBase in onpolicy says:
        # self.out_dim = args.gnn_hidden_size * (args.gnn_num_heads if args.gnn_concat_heads else 1)
        # Let's check onpolicy SimplifiedAttentionNet again.
        # It sums heads. So output is hidden_size.
        # So onpolicy GNNBase out_dim calculation might be wrong if it assumes concat but SimplifiedAttentionNet sums.
        # Or maybe SimplifiedAttentionNet in onpolicy handles concat?
        # In onpolicy code I read:
        # x = (head_outputs * alpha.unsqueeze(1)).sum(dim=-1)
        # This is weighted sum. So output is hidden_size.
        # So I should set out_dim = hidden_size.
