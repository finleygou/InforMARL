...
def get_config():
    ...
    parser.add_argument("--use_lstm", type=lambda x: bool(strtobool(x)), default=False, help="Whether to use LSTM for recurrent policy")
...
def graph_config(args, parser):
    ...
    parser.add_argument("--embedding_size", type=int, default=embedding_size, help="Embedding layer output size for each category")
    parser.add_argument("--embed_hidden_size", type=int, default=16, help="Hidden layer dimension after the embedding layer")
    parser.add_argument("--embed_layer_N", type=int, default=1, help="Number of hidden linear layers after the embedding layer")
    parser.add_argument("--embed_use_ReLU", action="store_false", default=True, help="Whether to use ReLU in the linear layers after the embedding layer")
    parser.add_argument("--embed_add_self_loop", action="store_true", default=False, help="Whether to add self loops in adjacency matrix")
    parser.add_argument("--gnn_hidden_size", type=int, default=16, help="Hidden layer dimension in the GNN")
    parser.add_argument("--gnn_num_heads", type=int, default=3, help="Number of heads in the transformer conv layer (GNN)")
    ...
    parser.add_argument("--gnn_layer_N", type=int, default=2, help="Number of GNN conv layers")
    parser.add_argument("--gnn_use_ReLU", action="store_false", default=True, help="Whether to use ReLU in GNN conv layers")
    parser.add_argument("--graph_feat_type", type=str, default="global", choices=["global", "relative"], help="Whether to use 'global' node/edge feats or 'relative'")
    parser.add_argument("--actor_graph_aggr", type=str, default="node", choices=["global", "node"], help="Whether we want to pull node specific features from the output or perform global_pool on all nodes.")
    parser.add_argument("--critic_graph_aggr", type=str, default="global", choices=["global", "node"], 
